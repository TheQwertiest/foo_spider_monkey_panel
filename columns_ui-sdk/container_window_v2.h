...
public:
    ...
    bool class_addref(const GUID& p_guid, WNDPROC wndproc, t_uint32 class_styles, LPWSTR cursor, HBRUSH bk_back,
        t_uint32 extra_wnd_bytes)
    {
        t_size index = NULL;
        if (m_classes.bsearch_t(window_class::g_compare_by_value, p_guid, index))
            ...
            m_classes.insert_item(
                new window_class(p_guid, wndproc, class_styles, cursor, bk_back, extra_wnd_bytes), index);
        }
    ...
    void class_deref_delayed(const GUID& p_guid)
    {
        ...
        if (core_api::are_services_available()) // useful check ??
            static_api_ptr_t<main_thread_callback_manager>()->add_callback(
                new service_impl_t<window_class_release_delayed>(p_guid));
    };
...
template <typename TBase>
HWND container_window_v2_t<TBase>::create(
    HWND wnd_parent, LPVOID create_param, const ui_helpers::window_position_t& p_window_position)
{
    ...
    if (g_window_class_manager.class_addref(ourGUID, (WNDPROC)g_on_message, get_class_styles(), get_class_cursor(),
            get_class_background(), get_class_extra_wnd_bytes())) {
        LPVOID createparams[2] = {this, create_param};
        m_wnd = CreateWindowEx(get_ex_styles(), print_guid_wide(get_class_guid()),
            pfc::stringcvt::string_wide_from_utf8(get_window_title()), get_styles(), p_window_position.x,
            p_window_position.y, p_window_position.cx, p_window_position.cy, wnd_parent, 0, core_api::get_my_instance(),
            &createparams);
    }
...
template <typename TBase>
HWND container_window_v2_t<TBase>::create_in_dialog_units(
...
LRESULT WINAPI container_window_v2_t<TBase>::g_on_message(HWND wnd, UINT msg, WPARAM wp, LPARAM lp)
{
    ...
    if (msg == WM_NCCREATE) {
        ...
        p_this = reinterpret_cast<container_window_v2_t<TBase>*>(
