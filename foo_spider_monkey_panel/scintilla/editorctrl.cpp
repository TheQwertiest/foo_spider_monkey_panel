...
{
    StringComparePartialNC( t_size len_ )
        : len( len_ )
    {
    }
...
bool CScriptEditorCtrl::StartCallTip()
{
    ...
    while ( m_nStartCalltipWord > 0 && ( IsIdentifierChar( line[m_nStartCalltipWord - 1] ) || ( line[m_nStartCalltipWord - 1] == '.' ) ) )
    {
        --m_nStartCalltipWord;
    }
...
void CScriptEditorCtrl::FillFunctionDefinition( int pos )
{
    ...
    if ( m_apis.get_count() )
    {
        ...
        if ( !GetNearestWords( words, m_szCurrentCallTipWord.get_ptr(), m_szCurrentCallTipWord.get_length(), "(" ) )
        {
...
void CScriptEditorCtrl::SetScintillaSettings()
{
    ...
    {
        int size = GetPropertyExpanded( propName.c_str(), 0 );
        if ( !size )
        {
            return std::nullopt;
        }

        std::string propValue( size, '\0' );
        GetPropertyExpanded( propName.c_str(), propValue.data() );
