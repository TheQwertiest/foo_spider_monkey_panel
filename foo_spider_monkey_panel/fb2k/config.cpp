...
bool PanelProperties::LoadJson( stream_reader& reader, abort_callback& abort, bool loadRawString )
{
    ...
    try
    {
        ...
        if ( loadRawString )
        {
            jsonStr = qwr::pfc_x::ReadRawString( reader, abort );
        }
        else
        {
            uint32_t version;
            ...
            jsonStr = qwr::pfc_x::ReadString( reader, abort );
        }
        ...
        for ( auto& [key, value]: jsonValues.items() )
        {
            ...
            values.emplace( qwr::unicode::ToWide( key ), std::make_shared<mozjs::SerializedJsValue>( serializedValue ) );
        }
    }
...
void PanelSettings::Load( stream_reader& reader, t_size size, abort_callback& abort )
{
    ...
    if ( size > sizeof( Version ) )
    {
        try
        {
            ...
            script = qwr::pfc_x::ReadString( reader, abort );
