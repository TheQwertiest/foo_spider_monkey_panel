...
public: // methods
    ...
    std::optional<JSObject*> CreateThemeManager( std::wstring classid );
    std::optional<JSObject*> CreateTooltip( std::wstring name, uint32_t pxSize, uint32_t style );
    std::optional<uint32_t> GetColourCUI( uint32_t type, std::wstring guidstr );
    ...
    //std::optional<JS::HandleValue> GetProperty( std::string name, VARIANT defaultval, VARIANT* p );
    ...
    //std::optional<std::nullptr_t> SetProperty( std::string name, JS::HandleValue val );
